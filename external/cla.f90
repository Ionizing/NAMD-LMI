...
!  Address: https://hachi.cee.pdx.edu/fossil/CLAF90/info/tip
!           https://ingria.ceoas.oregonstate.edu/fossil/CLAF90/doc/tip/README.md
!   
module cla
...
!      call cla_fatal("2020-10-23: POSITIONAL ARGUMENTS BUGGY. Turned off for now.")
    ...
    subroutine cla_validate_info(cmd_name,info)
      ...
      if (ncla == 0) then
         if (info) print *,"    ... none found. Returning."
         return
      end if
      ...
      if (info) print *,"    cla_command_argument_count = ",ncla
      do k=1,ncla
         call cla_get_command_argument(k,arg)
         if (info) print *,"    cla_command_argument #",k,": ",arg
      end do
      ! Positional arguments must all occur either before or after all the key/value arguments.
      ! Positional arguments are recognized by not containing a leading "-"
      kcla = 0
      do while (kcla < ncla)
         ...
         if (info) print *,"          positional arg #",kcla,"= ",trim(arg), &
         ...
         do kk=1,cla_num
            ...
            if (cla_key_arg_match(cla_registry(kk)%key, &
               ...
               if (cla_registry(kk)%kind == cla_flag) then
                  if (info) print *,"          key = ",trim(key)," is a flag"
                  kcla = kcla + 1
                  exit
               else
                  call cla_get_command_argument(kcla+2,value)
                  kcla = kcla + 2
                  if (info) print *,"          key, value ?= ",trim(key)," ",trim(value), &
      ...
      do while (kcla < ncla)
         ...
         if (info) print *,"    positional arg ?= ",arg
      end do
      if (info) print *,"    No errors found in syntax validation, but type/kind-validity not checked!"
      if (info) print *,"    If a -key value pair is repeated, the last one is used."
