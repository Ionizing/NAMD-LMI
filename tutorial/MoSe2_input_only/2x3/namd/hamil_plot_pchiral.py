...
class Hamil:
    ...
    def plot_pij_chiral(self, pngfname="hamil_pchiral.png"):
        pchiral = self.pij_t[:, 0, ...] + 1j * self.pij_t[:, 1, ...]

        pij = np.mean(np.abs(pchiral), axis=(0))
        np.fill_diagonal(pij, 0)
        ...
        if pij.shape[0] <= 15:
            for (i, j), z in np.ndenumerate(pij):
                color = "k" if z/zmax < 0.7 else "w"
                ax.text(j+0.5, i+0.5, "{:0.2f}".format(z), ha="center", va="center",
                        fontsize="x-large", color=color)

        fig.suptitle("<i|px+ipy|j> in {}".format(self.fname))
        fig.tight_layout(pad=0.5)
        print("Writing {}".format(pngfname))
        fig.savefig(pngfname, dpi=400)
        pass
